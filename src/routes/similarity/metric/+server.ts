...
export async function GET({ url }) {
    console.log("Received similarity metric request:", url);
    ...
    try {
        ...
        if (!(await driver.executeQuery("CALL gds.graph.exists('similarity') YIELD exists return exists;", {}, { database: NEO4J_DATABASE })).records[0].get('exists')) {
            await driver.executeQuery(
                `WITH apoc.map.fromPairs($clustering) AS communities
                 MATCH (f0:Findspot)-[r:HAS_SIMILAR_COINS]->(f1:Findspot)
                 RETURN gds.graph.project('similarity', f0, f1,
                 { sourceNodeProperties: f0 { community: toInteger(communities[elementId(f0)])},
                   ...
                   relationshipProperties: r { .sqrt_score } },
        ...
        let { records, summary } = await driver.executeQuery(
            `CALL gds.modularity.stream('similarity', { communityProperty: 'community', relationshipWeightProperty: 'sqrt_score' })
        ...
        if ((await driver.executeQuery("CALL gds.graph.exists('similarity') YIELD exists return exists;", {}, { database: NEO4J_DATABASE })).records[0].get('exists')) {
            await driver.executeQuery(
                `CALL gds.graph.drop('similarity') YIELD graphName;`,
                {},
                { database: NEO4J_DATABASE }
            );
        }
