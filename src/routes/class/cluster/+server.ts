...
export async function GET({ url }) {
    console.log("Received type clustering request:", url);
    ...
    try {
        const coinClass = url.searchParams.get('coinClass');
        if (coinClass == null) {
            return json([]);
        }

        if (!(await driver.executeQuery("CALL gds.graph.exists('shared_types') YIELD exists return exists;", {}, { database: NEO4J_DATABASE })).records[0].get('exists')) {
            await driver.executeQuery(
                `MATCH (f0:Findspot)-[r:SHARES_TYPES]->(f1:Findspot) RETURN gds.graph.project('shared_types', f0, f1, { relationshipProperties: r {shared: r[$coinClass]} }, { undirectedRelationshipTypes: ['*'] })`,
                { coinClass: coinClass },
        ...
        let { records, summary } = await driver.executeQuery(
            `CALL gds.modularityOptimization.stream('shared_types', { relationshipWeightProperty: 'shared' }) YIELD nodeId, communityId RETURN elementId(gds.util.asNode(nodeId)) AS id, communityId AS cluster`,
            {},
            { database: NEO4J_DATABASE }
        );
        ...
        if ((await driver.executeQuery("CALL gds.graph.exists('shared_types') YIELD exists return exists;", {}, { database: NEO4J_DATABASE })).records[0].get('exists')) {
            await driver.executeQuery(
                `CALL gds.graph.drop('shared_types') YIELD graphName;`,
                {},
                { database: NEO4J_DATABASE }
            );
        }
