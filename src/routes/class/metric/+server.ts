        ...
        if (!(await driver.executeQuery("CALL gds.graph.exists('shared_types') YIELD exists return exists;", {}, { database: NEO4J_DATABASE })).records[0].get('exists')) {
            await driver.executeQuery(
                `WITH apoc.map.fromPairs($clustering) AS communities
                 MATCH (f0:Findspot)-[r:SHARES_TYPES]->(f1:Findspot) RETURN gds.graph.project('shared_types', f0, f1,
                 { sourceNodeProperties: f0 { community: toInteger(communities[elementId(f0)])},
                   ...
                   relationshipProperties: r {shared: r[$coinClass]} },
                 { undirectedRelationshipTypes: ['*'] })`,
                { coinClass: coinClass, clustering: clustering },
                { database: NEO4J_DATABASE }
            );
        }

        let { records, summary } = await driver.executeQuery(
            `CALL gds.modularity.stream('shared_types', { communityProperty: 'community', relationshipWeightProperty: 'shared' })
